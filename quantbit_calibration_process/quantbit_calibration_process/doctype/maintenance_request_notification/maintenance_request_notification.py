...
class MaintenanceRequestNotification(Document):
	...
	def on_submit(self):
		if self.notification_calibration_task_list:
			if not any(row.check for row in self.get("notification_calibration_task_list")):
				frappe.throw("At least one row in the table must have the 'Check' field selected.")
	...
	def create_maintanance_orders(self):
		...
		doc.equipment_maintanance_schedule = self.equipment_maintenance_schedule
		doc.order_type=self.request_type
		if self.schedule_date:
			doc.maintanance_date = self.schedule_date 
