...
def cm():
	...
	child_data = frappe.get_all("Schedule Of Item",filters = {"is_requested":0,"docstatus":1},fields=["name","parent","scheduled_date","is_requested"],)
	if(child_data):
		for i in child_data:		
			doc1 = frappe.get_doc('Equipment Maintanance Schedule', i.parent)
			if doc1:				
				scheduled_date = frappe.utils.data.getdate(i.scheduled_date)
				if not frappe.db.exists("Maintenance Request-Notification", {"schedule_date": i.scheduled_date, "equipment_id":doc1.equipment_id,"equipment_maintenance_schedule":i.parent}):
				# frappe.throw(str((scheduled_date - current_date).days))
					if (scheduled_date - current_date).days <= doc1.days_toschedule:
						...
						calibration_tasks = frappe.get_all("Calibration Task List", filters={"parent": doc1.equipment_id}, fields=["parameter", "parameter_details", "value"])
						if calibration_tasks:
							for task in calibration_tasks:
								doc.append("notification_calibration_task_list", {
									"parameter": task.parameter,
									"parameter_details": task.parameter_details,
									"value": task.value
								})
