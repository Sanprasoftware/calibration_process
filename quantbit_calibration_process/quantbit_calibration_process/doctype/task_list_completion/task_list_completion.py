...
class TaskListCompletion(Document):
	...
	def on_update(self):
		if not self.task_completion_details and self.completion_status == "Fully Completed":
			frappe.throw("Please Enter Result Before Selecting Fully Completed")
	...
	def check_all_complete(self):
		if self.completion_status:
			doc = frappe.get_doc('Equipment Maintenance Order', self.maintenance_order)
			sched_item = frappe.get_value('Schedule Of Item',{'parent':self.equipment_maintenance_schedule,'scheduled_date':self.maintanance_scheduled_date,'item_code':self.equipment_id},"name")
			...
			if all_tasks_completed:
				...
				if sched_item:
					frappe.db.set_value('Schedule Of Item',sched_item,'completion_status',"Fully Completed")

			else:
				frappe.db.set_value('Equipment Maintenance Order', self.maintenance_order, 'status', 'Open')
				if sched_item:
					frappe.db.set_value('Schedule Of Item',sched_item,'completion_status',"Partially Completed")
